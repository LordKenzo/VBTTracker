...
//  ✅ AGGIORNATO: Filtro impostazioni per tipo sensore
//
...
struct RepDetectionSettingsView: View {
    ...
    var body: some View {
        List {
            ...
            // ✅ Sezioni COMUNI (Arduino + WitMotion)
            romSection

            // ✅ Sezioni SOLO WitMotion
    ...
    private var sensorWarning: String {
        ...
        case .arduino:
            return "Configurazione semplificata per sensore Arduino. Il rilevamento si basa su distanza e velocità misurate direttamente."
        }
    }
    ...
    private var romFooter: Text {
        if settings.selectedSensorType == .arduino {
            if settings.useCustomROM {
                return Text("ROM misurato direttamente dal sensore laser. Le ripetizioni fuori dal range ±tolleranza verranno scartate.")
            } else {
                return Text("Abilita per validare il ROM delle ripetizioni. Il sensore Arduino misura direttamente la distanza.")
            }
        } else {
            if settings.useCustomROM {
                return Text("Distanza petto-braccio durante bench press. Le ripetizioni verranno conteggiate solo se il displacement rientra nel range ±tolleranza. Attivo solo con frequenza campionamento ≥60Hz.")
            } else {
    ...
    private var arduinoLookAheadSection: some View {
        Section {
            ...
            Text("Tempo di attesa per confermare il completamento della ripetizione. Valori più alti riducono i falsi positivi ma aumentano il ritardo.")
        }
    }
